...
client.on(Events.MessageCreate, async (message) => {
    ...
    try {
        ...
        for (const [gatilho, resposta] of Object.entries(respostas)) {
            if (lower.includes(gatilho.toLowerCase())) { await message.reply(resposta); break; }
        }
    } catch(_) {}
    ...
    if (!seg?.ativo) return;
    if (message.member.roles.cache.has(config.cargos?.staff)) return;
    if (message.author.id === config.ownerId) return;
    let motivo = null;
    if (seg.banirImagens && message.attachments.size > 0) motivo = 'Imagem não autorizada';
...
async function registerCommands() {
    ...
    } catch (err) { console.error('❌ Erro ao registrar commands:', err.message); }
}
    ...
    setInterval(async () => {
        ...
        for (const guild of client.guilds.cache.values()) {
            try {
                ...
                if (!cfg.diasDaSemana.includes(agora.getDay())) continue;
